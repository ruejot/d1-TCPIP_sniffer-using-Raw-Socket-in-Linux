...
int main(int argc, char *argv[])
{
  ...
  for (;;)
  {
    ...
    ptemp += sizeof(struct ip); //shift ip header length, then point to next layer header(TCP)
    ...
    case IPPROTO_TCP:
      ptcp = (struct tcphdr *)ptemp; //shift pointer(ptcp) to tcp header
      ...
      if (strcmp(s_macaddr, mymacaddr) != 0 && strcmp(d_macaddr, mymacaddr) != 0 && strcmp(d_macaddr, s_macaddr) != 0)
      {
        ...
        printf("IP->src_ip =%s,\tsrc_port:%d \n", inet_ntoa(*(struct in_addr *)&(pip->ip_src)), ntohs(ptcp->th_sport));
        printf("IP->dst_ip =%s,\tdst_port:%d \n", inet_ntoa(*(struct in_addr *)&(pip->ip_dst)), ntohs(ptcp->th_dport));
        tcpnum++;
      }
      break;
    }
